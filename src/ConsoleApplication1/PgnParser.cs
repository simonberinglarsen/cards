...
namespace ConsoleApplication1
{
    public class PgnParser
    {
        ...
        public void Parse()
        {
            ...
            string game = "";
            int i = 0;
            while (i < _pgnText.Length)
            {
                char ch = _pgnText[i];
                if (ch == '[')
                {
                    int j = _pgnText.IndexOf(']', i);
                    AddHeader(_pgnText.Substring(i + 1, j - i), headers);
                    i = j + 1;
                    continue;
                }
                else if (ch == '1')
                {
                    game = _pgnText.Substring(i);
                    break;
                }
                i++;
            }
            Game = RemoveComments(game);
            if (Game[0] == '.')
            {
                int k = 8;
            }
        ...
        private void AddHeader(string headerText, Dictionary<string, string> headers)
        {
            int i = headerText.IndexOf('\"') + 1;
            int j = headerText.LastIndexOf('\"');
            string key = headerText.Substring(0, i - 1).Trim().ToUpper();
            string value = headerText.Substring(i, j - i).Trim();
            headers.Add(key, value);
        }

        // source: http://www.chessgames.com/chessecohelp.html
        private string ecoList = @"ECO Chess Opening Codes:
...
1 d4 Nf6 2 c4 e6 3 Nc3 Bb4 4 e3 O-O 5 Nf3 d5 6 Bd3 c5 7 O-O Nc6 8 a3 dxc4 9 Bxc4
